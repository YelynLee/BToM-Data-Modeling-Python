...
for t=1:path_len
    ...
    % ====================================================================
    % 🚨 [Lesion: Belief Update 무력화] 🚨
    % 에이전트의 시야 확률(o_score1)을 모두 1로 강제 변경하여, 
    % 새로운 위치에서 무엇을 보든 얻는 정보량(Information Gain)을 0으로 만듦.
    % 이로 인해 에이전트는 초기 믿음을 끝까지 업데이트하지 않게 됨.
    % ====================================================================
    o_score1_lesion = ones(size(o_score1)); 
    co_score1 = repmat(o_score1_lesion',[1 1 n_action]) .*repmat(reshape(c_score1,[n_world 1 n_action]),[1 n_obs 1]);
